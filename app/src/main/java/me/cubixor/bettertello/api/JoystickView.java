    ...
    */
    ...
    public JoystickView(Context context, AttributeSet attrs) {
        ...
        try {
            ...
            left = styledAttributes.getBoolean(R.styleable.JoystickView_JV_left, true);
        } finally {
    ...
    private void initPosition() {
        ...
        mCenterY = mPosY = getHeight() / 2;

        posX = mFixedCenterX = left ? getWidth() / 3 : getWidth() - (getWidth() / 3);
        posY = mFixedCenterY = getHeight() / 2;
    }
    ...
    protected void onDraw(Canvas canvas) {
        // Draw the background
        canvas.drawCircle(posX, posY, mBackgroundRadius, mPaintBackground);

        // Draw the circle border
        canvas.drawCircle(posX, posY, mBorderRadius, mPaintCircleBorder);

        // Draw the button from image
        if (mButtonBitmap != null) {
            canvas.drawBitmap(
                    mButtonBitmap,
                    mPosX + posX - mCenterX - mButtonRadius,
                    mPosY + posY - mCenterY - mButtonRadius,
                    mPaintBitmapButton
            );
        }
    ...
    public float getNormalizedX() {
        ...
        return Math.round((float) (posX - mPosX) / -mBorderRadius * 100f) / 100f;

        //return Math.round((posX - mPosX - mButtonRadius) * 100.0f / (getWidth() - mButtonRadius * 2));
    }
    ...
    public float getNormalizedY() {
        if (posY == mFixedCenterY) {
            return 0;
        }

        return Math.round((float) (posY - mPosY) / mBorderRadius * 100f) / 100f;
